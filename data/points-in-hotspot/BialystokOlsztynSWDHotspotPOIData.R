...
BialystokOlsztynSWDHotspotPOIData <- setRefClass(
  Class="BialystokOlsztynSWDHotspotPOIData",
  fields=list(
  ...
  methods = list(
    ...
    getCityFilePath = function(cityName) {
      if(cityName == 'bialystok') {
        filePath <- const$bialystokSWDHotSpotPath
      } else if(cityName == 'olsztyn') {
        filePath <- const$olsztynSWDHotSpotPath
      }
      filePath
    },
