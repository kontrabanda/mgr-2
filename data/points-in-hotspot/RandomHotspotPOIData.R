...
RandomHotspotPOIData <- setRefClass(
  Class="RandomHotspotPOIData",
  fields=list(
  ...
  methods = list(
    ...
    readData = function() {
      read.csv(const$randomHotSpotPath, sep = ",")
    },
    extractCategories = function() {
      c('bialystokRandom', 'bournemouthRandom')
    },
    getData = function(category) {
      categoryPath <- paste(const$randomHotSpotPath, 'poi', r, category, sep = '/')
